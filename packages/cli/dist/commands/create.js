...
async function createApp(projectName, options = {}) {
  console.log(chalk.cyan('\n🚀 Creating Aether.js application...\n'));
...
async function generateAppFiles(projectPath, template, options) {
  ...
  switch (template) {
    case 'enterprise':
      await generateEnterpriseFiles();
      break;
    case 'pwa':
      await generatePWAFiles();
      break;
    case 'spatial':
      await generateSpatialFiles();
      break;
    case 'ai':
      await generateAIFiles();
      break;
    case 'research':
      await generateResearchFiles();
      break;
    default:
      await generateBasicFiles();
  }
}
...
function generateMainApp(template, options) {
  const features = options.features || [];
  const hasRevolutionary = features.length > 0 || options.revolutionary;
  
  let imports = `import { AetherComponent, PersistentStore, SyncEngine } from '@aether/framework';\n`;
  
  if (hasRevolutionary) {
    imports += `import { AetherRevolutionary } from '@aether/framework';\n`;
  }
