...
describe('Load Testing', () => {
  ...
  describe('Memory Stress Testing', () => {
    ...
    test('should handle memory cleanup properly', async () => {
      ...
      // Allow small measurement noise in CI environments
      const roundedRecovered = Math.round(memoryRecovered * 100) / 100;
      expect(roundedRecovered).toBeGreaterThanOrEqual(0);
    });
  });

  describe('Sync Engine Load Testing', () => {
    // Increase timeout for heavy load test in CI environments
    ...
    test('should handle high-frequency sync operations', async () => {
      ...
      // Mock fast per-mutation sync to keep the load test deterministic and fast
      let syncCallCount = 0;
      syncEngine.syncMutation = jest.fn().mockImplementation(() => {
        syncCallCount++;
      ...
      // Process queue (carbon-aware path); force immediate to avoid waiting in tests
      await syncEngine.forceSync();

      const endTime = performance.now();
      const duration = endTime - startTime;
      // In the forceSync path we sync per mutation; expect one call per mutation
      const expectedBatches = mutationCount;
    ...
    test('should handle massive task scheduling', async () => {
      ...
      const tasks = Array.from({ length: taskCount }, (_, i) => {
        ...
        return scheduler.scheduleTask(
          {
            type: 'test-task',
            execute: () => Promise.resolve(`task-${i}-result`),
            estimatedDuration: 1
          },
          priority,
          { carbonThreshold }
